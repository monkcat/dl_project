#!/usr/bin/env bash
# Resume after Wave 1 segfault: a, e already trained → eval-only;
# then continue waves 2-8 normally.
set -euo pipefail
cd "$(dirname "$0")"
mkdir -p logs ckpt eval/results/experiments

launch() {
    # Usage: launch <row_id> <gpu_id> [extra_args...]
...
echo "=== Wave 1 (eval-only): a + e ===" >&2
launch a 0 --skip_training --ckpt ckpt/a_baseline_infonce.pt; A=$!
launch e 1 --skip_training --ckpt ckpt/e_grcl_no_gpe.pt;     E=$!
