...
multimodal retrieval over academic papers, in which the document's
...
$33.1$ to $\mathbf{63.3}$ ($+30$pp) \emph{without any fine-tuning}---the
study's largest single effect.
\end{abstract}
...
related elements rather than a single one: a figure, its caption, the
citing paragraph, and an appendix reference, linked through typed
relations like \texttt{caption\_of} and \texttt{refer\_to}. Two
properties of this answer matter for retrieval: it spans
\emph{multiple modalities} (visual figures, captions, body text), and
it lives at the level of \emph{individual elements} rather than whole
pages. Page-level visual RAG~\cite{colpali,visrag} therefore returns
coarse units that cannot pinpoint which element on a page is the
evidence and tends to miss cross-page references; element-level
retrieval without a graph prior ranks each candidate independently and
fails to recover elements that are bound only through structural
relations.
