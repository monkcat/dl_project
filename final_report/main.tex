...
multimodal encoders perform unevenly on academic visual elements:
compact dual encoders such as CLIP and SigLIP~\cite{siglip2} score
poorly on figure/table retrieval, while retrieval-tuned MLLMs like
GME-Qwen2-VL-2B~\cite{gme} reach usable accuracy. Second, even when the
retrieval target is a single element, it rarely stands alone---a figure
is bound to its caption, the citing paragraphs, and sometimes an
appendix reference through typed relations like \texttt{caption\_of}
and \texttt{refer\_to}. Page-level visual RAG~\cite{colpali,visrag}
returns coarse units that cannot pinpoint the element evidence;
element-level retrieval without a graph prior ignores these typed
relations.
