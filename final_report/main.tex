...
\title{Structure-Aware Element-level Retrieval\\for Multimodal RAG in Academic Documents}
...
Each document becomes a per-document graph $G_d\!=\!(V_d,E_d)$ with six
...
Intuitively, a caption that physically accompanies a figure carries the
tightest binding (weight $1.0$); an explicit paragraph reference
($\backslash$ref) is slightly weaker ($0.8$); structural co-occurrence
within a section, or mere reading adjacency, are weaker still. Two
...
cross-modal modifier ($1.1$ for \texttt{refer\_to} into a
figure/table) slightly boosts paragraph-to-visual references over
paragraph-to-paragraph ones.
...
\label{eq:kernel}
\end{equation}
where each $w_{uv}$ is the BASE schema with target-modifier corrections.
Intuitively, $g(q,e)$ is close to one when $q$ and $e$ are directly
bound by a strong edge (e.g., a figure and its caption), shrinks for
two-hop paths (e.g., a body paragraph reaching a figure through a
referenced caption), and is zero when no short path exists.
...
retriever without parameter updates---in particular, an external MLLM
that has no notion of our schema.
