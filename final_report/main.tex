...
Given any base retriever's scores $s\!\in\!\mathbb{R}^{|V_d|}$, we
truncate to the top-$N\!=\!50$ candidates, build a $50\!\times\!50$
transition matrix $P$ by row-normalizing the \emph{same} edge-weight
schema as the training kernel, and iteratively re-rank with one of
three regimes: \emph{uniform diffusion} sets all edge weights to one
(structure-only); \emph{weighted diffusion} keeps the full BASE
weights; both update as
$s^{(t+1)}\!=\!(1\!-\!\alpha)\,s^{(t)}+\alpha\,P^\top s^{(t)}$ for $T$
steps. \emph{Personalized PageRank} re-injects the original scores
as a teleport,
$s^{(t+1)}\!=\!\alpha\,q+(1\!-\!\alpha)\,P^\top s^{(t)}$ with
$q\!=\!s^{(0)}$~\cite{ppr}, iterating until
$\|s^{(t)}\!-\!s^{(t-1)}\|_\infty\!<\!10^{-4}$. One iteration is $<\!1$\,ms,
and all three regimes are eval-only and apply to any base retriever
